...
SELECT *, (TotalDeaths/TotalCases)*100 AS CaseFatalityRate FROM #TotalInTheWorld

--PER COUNTRY (include continent)
SELECT location, population, TotalCases, TotalDeaths, (TotalDeaths/TotalCases) as CaseFatalityRate
FROM 
	(
...
WHERE population is not null and CaseFatalityRate is not null




--Attack Rate (RATIO BETWEEN TOTALCASES AND POPULATION)

--World
SELECT  location, population, date, HighestInfectionCount, AttackRate
FROM
(	SELECT
	location, date, population, MAX(total_cases) as HighestInfectionCount, (MAX(total_cases)/population)*100 as AttackRate
	FROM PortfolioProject..CovidDeaths
	GROUP BY location, population, date
) AS Subquery
WHERE population is not null and HighestInfectionCount is not null

--Country (include continent)
SELECT location, population, HighestInfectionCount, AttackRate
FROM
(	SELECT
	location, population, MAX(total_cases) as HighestInfectionCount, (MAX(total_cases)/population)*100 as AttackRate
	FROM PortfolioProject..CovidDeaths
	GROUP BY location, population
) AS Subquery
WHERE population is not null and HighestInfectionCount is not null

--Mortality Rate (RATIO BETWEEN TOTALDEATHS AND POPULATION)

--WORLD
SELECT 
		SUM(Population) AS TotalPopulation,
		SUM(TotalDeath) AS TotalDeaths,
		(SUM(TotalDeath)/SUM(Population)) *100 as MortalityRate
FROM	
(
		SELECT continent, 
...
--PER COUNTRY
SELECT location, population, TotalDeathCount, (TotalDeathCount/population) * 100 as MortalityRate
FROM 
